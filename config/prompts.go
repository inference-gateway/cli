...
Each subagent is independent and cannot itself spawn further subagents. Prefer narrow, self-contained task descriptions.`,
		...
		RecordStart: PromptsToolDescription{
			Description: `Starts recording the screen to an MP4 file (H.264) and returns immediately with the file path and the captured rectangle. Record the entire primary screen (mode "screen", the default), a single application window (mode "window" with window: frontmost, app:<name>, pid:<number>, or an application name - the window's bounds are taken when the recording starts), or a region (mode "region" with x/y/width/height in the frame coordinate space, the same space as Computer screenshots). Only one recording runs at a time. It stops on its own at the configured max duration; call RecordStop in this same session to finish it and get the duration and size. Use it to keep an audit trail of what you do on the machine or to record a demonstration.`,
		},
		RecordStop: PromptsToolDescription{
			Description: `Stops the screen recording started with RecordStart, finalizes the MP4 file, and returns its path, duration, and size. Also returns the file of a recording that already stopped at its max duration. Takes no arguments.`,
		},
		...
		ImageDecode: PromptsToolDescription{
			Description: `Looks at a local image file or http(s) URL. If you can see images, the image itself is attached to the next message; otherwise (or additionally, when a vision annotator is configured) you get a text description: a scene summary plus a numbered list of detected elements (label, text, bounding box). Use it to inspect any image on disk or from a URL - e.g. video keyframes, outputs of ImageGeneration/ImageEdit under ~/.infer/projects/<project-slug>/artifacts, @-referenced image paths, downloaded files, or remote images. Pass an optional prompt to ask a specific question about the image; the summary then answers it. Read-only, does not require approval.`,
		},
...
Cost control: quality defaults to "low" and size to "1024x1024". Keep those defaults unless the user explicitly asks for higher quality or a different size - higher tiers cost significantly more and take longer. "make it nicer" is not an explicit request for high quality; "high quality", "hd", "1536x1024" are.`,
		...
		TextToSpeech: PromptsToolDescription{
			Description: `Synthesize speech from text and save it as an MP3 file. Returns the saved file path.

Pass only text for a stock voice. Pass voice_sample to clone a voice: a bare file name (no directories, no absolute paths) of a WAV in the working directory, ideally 10-30 seconds of clean single-speaker speech. Pass output_path to name the file: also a bare file name only - the MP3 is ALWAYS written into the configured output directory, never the working directory or any path you choose, so never pass directories or absolute paths; omit it for a timestamped name. To place the audio elsewhere, synthesize first and then copy the returned file.
...
The first call may be slow (engine warmup or model download); if it fails with a not-ready or temporarily-unavailable error, wait a few seconds and retry once. Do not claim the audio was played aloud - it is written to disk for the user to open, at the returned path.`,
		...
		TextToMusic: PromptsToolDescription{
			Description: `Compose a music clip from a text prompt and save it as an MP3 file. Returns the saved file path.
...
Generation can take a while (music models are slow); if it fails with a temporarily-unavailable error, wait a few seconds and retry once. Do not claim the clip was played aloud - it is written to disk for the user to open, at the returned path.`,
		...
		TextToSFX: PromptsToolDescription{
			Description: `Generate a short sound effect or ambience clip from a text prompt and save it as an MP3 file. Returns the saved file path.
