...
- command: re-run a check command server-side at a fixed interval until it exits 0 (e.g. "curl -sf localhost:8080/health"). The check command goes through the same bash allow-list as the Bash tool.

Every mode requires timeout_seconds (bounded by the config ceiling). Returns a structured result: which condition fired (or timeout), elapsed time, and for shells the exit codes and tail output.
