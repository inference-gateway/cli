...
Each subagent is independent and cannot itself spawn further subagents. Prefer narrow, self-contained task descriptions.`,
		...
		ActivateApp: PromptsToolDescription{
			Description: `Brings a running application to the foreground. Provide either "app_id" (the stable identifier returned by GetFocusedApp or ListRunning, e.g. "com.google.Chrome" or "pid:1234") or "name" (a human-readable name substring, e.g. "Chrome" or "Terminal") to identify the target. Name matching is case-insensitive and selects the first matching running application. Use GetFocusedApp first to check current state. Works on macOS, X11, and (where supported) Wayland.`,
		},
