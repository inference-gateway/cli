...
**Entry points** (`main.go` → `cmd/root.go`): `initConfig()` (in `cmd/root.go`) layers config in this order — built-in defaults → `~/.infer/config.yaml` → `./.infer/config.yaml` → flags → `INFER_*` env vars (highest). Two env vars get special list-parsing because viper can't handle them generically: `INFER_A2A_AGENTS` and `INFER_TOOLS_BASH_WHITELIST_COMMANDS` (split on `,` or newline). The whitelist commands var **replaces** the resolved list; its `_APPEND` sibling — `INFER_TOOLS_BASH_WHITELIST_COMMANDS_APPEND` — instead **merges onto** the fully-resolved default/config/replace list (applied after `ReadInConfig`), so callers can add per-repo entries without clobbering the built-in default. Both whitelist vars are also exposed as persistent flags (`--tools-bash-whitelist-commands` and `--tools-bash-whitelist-commands-append`); for each the env var takes precedence over the matching flag (`resolveWhitelistOverride` in `cmd/root.go` resolves both).
...
- **Bash whitelist unified model** (`config/bash_whitelist.go`): a single `commands` list replaces the old commands+patterns split. Each entry is classified by `isBashEntryRegex`: a bare token (no space, no regex metacharacters `^$*+?()[]{}|\\`) matches the command **exactly** (e.g. `gh` allows only `gh`, never `gh issue list`), while entries with spaces or metacharacters are evaluated as Go regexes. All default entries are now regexes (`^echo( |$)` instead of `echo`) so they continue allowing arguments. `INFER_TOOLS_BASH_WHITELIST_PATTERNS` no longer exists; use `_COMMANDS` and `_COMMANDS_APPEND` only. The CLI default is the single source of truth; `infer-action` and the org reusable workflow are pure pass-throughs. The allow-list only ever *matches* a command; orthogonal **structural deny rules** in the same file reject a command before matching regardless of the list (they are security invariants, not config): command substitution (`$(...)`, backticks, `<()`/`>()`), file-write redirection (`>`/`>>` to a real file; benign `2>&1`/`>/dev/null` are stripped first), and dangerous `find` actions (`-exec`/`-delete`/…). **Env-var leak guard**: `$VAR`/`${VAR}` may be *used* in any command, but a command that prints (`echo`/`printf`) or publishes (`gh issue/pr create|comment|edit`) its arguments may not expand one — so `echo $AWS_SECRET_ACCESS_KEY` and `gh issue comment --body $TOKEN` are blocked while `ls $DIR`/`git log $REF` stay allowed; a single-quoted or backslash-escaped `$` is always literal (`outputCommandRe` + `containsVariableExpansion`). *Setting* env vars (`export FOO=bar`, `FOO=bar cmd`) is deliberately not auto-approved — it falls through to approval like any unlisted command. **Single-command policy**: any top-level shell operator (`|`, `|&`, `&&`, `||`, `;`, `&`, newline) makes the whole command non-whitelisted, so chains/pipelines always fall through to approval even when every segment is individually allowed (`echo x | echo` is *not* auto-approved) — operators inside quotes (a jq `'… | …'` or a `--title "a && b"`) don't count. `git push` is intentionally absent from the default list, so it always requires approval; users opt specific targets in with an anchored regex. `BashWhitelistRejectionHint` turns each deny rule into actionable feedback for the model.
