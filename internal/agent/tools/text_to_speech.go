...
func (t *TextToSpeechTool) resolveOutputPath(raw string) (string, error) {
	...
	return resolveMediaOutputPath(dir, "speech-", ".wav", raw)
}
...
// allocates a unique timestamped target with extension ext under dir instead.
func resolveMediaOutputPath(dir, prefix, ext, raw string) (string, error) {
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return "", fmt.Errorf("creating output directory: %w", err)
	}
	name := strings.TrimSpace(raw)
	if name == "" {
		file, err := os.CreateTemp(dir, prefix+time.Now().Format("20060102-150405")+"-*"+ext)
		if err != nil {
