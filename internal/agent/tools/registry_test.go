...
func TestRegistry_NewRegistry_DoesNotBlockOnMCP(t *testing.T) {
	...
	go func() {
		done <- NewRegistry(cfg, nil, nil, nil, nil, blocker, nil, nil, nil, nil)
	}()
...
func TestRegistry_ConcurrentMCPToolAccess(t *testing.T) {
	...
	registry := NewRegistry(cfg, nil, nil, nil, nil, &stubMCPManager{client: &agentdomainmocks.FakeMCPClient{}}, nil, nil, nil, nil)
