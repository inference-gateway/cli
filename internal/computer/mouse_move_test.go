...
func TestMouseMoveTool_CoordinateScaling(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "LLM execution - mismatched aspect ratios (aspect-preserving fit)",
			apiWidth:      1024,
			apiHeight:     768,
			logicalWidth:  1728,
			logicalHeight: 1117,
			...
			expectedX:     1728,
			expectedY:     1117,
			description:   "A 16:10 screen fits the target box as 1024x662; its bottom-right corner maps to the screen's bottom-right corner",
		},
		{
