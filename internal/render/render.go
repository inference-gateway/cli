...
func RenderJSON(events <-chan domain.ChatEvent, w io.Writer, sessionID, model string, cfg *config.Config, repo domain.ConversationRepository) error {
	...
	for event := range events {
		...
		case domain.ChatErrorEvent:
			...
			return fmt.Errorf("agent error: %w", e.Error)
...
// non-interactive chat output style.
...
// RenderAGUI renders events as newline-delimited AG-UI protocol events.
func RenderAGUI(events <-chan domain.ChatEvent, w io.Writer, sessionID, model string) error {
	...
	for event := range events {
		...
		case domain.ChatCompleteEvent:
			...
			if ev.Cancelled {
				e.emitRunError("cancelled")
			} else {
				e.emitRunFinished()
			}
		case domain.ChatErrorEvent:
			e.emitRunError(ev.Error.Error())
