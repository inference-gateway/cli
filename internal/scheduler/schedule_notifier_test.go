...
func TestScheduleNotifier_RecordOnlyJobIsIgnored(t *testing.T) {
	ch := &notifierFakeChannel{name: "telegram"}
	n := newNotifierWithChannel(ch)
	job := scheddomain.ScheduledJob{ID: "j1"}

	n.Notify(job, scheddomain.RunEvent{Line: []byte(`{"role":"assistant","content":"hello"}`)})
	n.Notify(job, scheddomain.RunEvent{Done: true, Err: errors.New("boom")})
...
func TestScheduleNotifier_FailureEvent(t *testing.T) {
	ch := &notifierFakeChannel{name: "telegram"}
	n := newNotifierWithChannel(ch)
	job := scheddomain.ScheduledJob{ID: "j1", Name: "morning report", Channel: "telegram", RecipientID: "user1"}

	n.Notify(job, scheddomain.RunEvent{Done: true, Err: errors.New("agent exploded")})
