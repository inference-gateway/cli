...
type TaskRetentionService struct {
	tasks        []scheddomain.TaskInfo
	maxRetention int
	mutex        sync.RWMutex
}

// Compile-time assertion that TaskRetentionService implements scheddomain.TaskRetentionService interface
var _ scheddomain.TaskRetentionService = (*TaskRetentionService)(nil)
...
func NewTaskRetentionService(maxRetention int) *TaskRetentionService {
	return &TaskRetentionService{
		tasks:        make([]scheddomain.TaskInfo, 0, maxRetention),
...
func (t *TaskRetentionService) AddTask(task scheddomain.TaskInfo) {
	...
	t.tasks = append([]scheddomain.TaskInfo{task}, t.tasks...)
...
func (t *TaskRetentionService) GetTasks() []scheddomain.TaskInfo {
	...
	result := make([]scheddomain.TaskInfo, len(t.tasks))
	copy(result, t.tasks)
	return result
}
...
func (t *TaskRetentionService) Clear() {
	t.mutex.Lock()
	defer t.mutex.Unlock()
	t.tasks = make([]scheddomain.TaskInfo, 0, t.maxRetention)
}
