...
type Supervisor struct {
	...
	retentionByKind map[scheddomain.JobKind]int
...
type supervised struct {
	job         scheddomain.BackgroundJob
	meta        scheddomain.JobMeta
	status      scheddomain.JobStatus
...
func NewSupervisor(messageQueue convdomain.MessageQueue, conversationRepo convdomain.ConversationRepository, notifier agentdomain.UINotifier) *Supervisor {
	...
	return &Supervisor{
		...
		retentionByKind:  make(map[scheddomain.JobKind]int),
...
func (s *Supervisor) SetRetentionCount(kind scheddomain.JobKind, max int) {
	s.mu.Lock()
...
func (s *Supervisor) Submit(job scheddomain.BackgroundJob) {
	if job == nil {
		return
	}
	meta := job.Meta()
	...
	var toClose scheddomain.BackgroundJob
	...
	sj := &supervised{job: job, meta: meta, status: scheddomain.JobRunning, cancel: cancel}
	s.jobs[meta.ID] = sj
	s.wg.Add(1)
	s.mu.Unlock()
...
func (s *Supervisor) monitor(ctx context.Context, sj *supervised) {
	...
	result := func() (r agentdomain.ToolExecutionResult) {
		...
		return sj.job.Run(ctx, func(sig scheddomain.JobSignal) { s.onSignal(sj, sig) })
	}()
...
func (s *Supervisor) evictOverCapLocked(kind scheddomain.JobKind) []*supervised {
...
// tool result.
func (s *Supervisor) formatResult(job scheddomain.BackgroundJob, meta scheddomain.JobMeta, result *agentdomain.ToolExecutionResult) string {
	var body string
...
func (s *Supervisor) Wind(id string, sig scheddomain.WindSignal) error {
	s.mu.RLock()
	sj := s.jobs[id]
	s.mu.RUnlock()
	if sj == nil {
...
func (s *Supervisor) WindAll(sig scheddomain.WindSignal) {
	s.mu.RLock()
	targets := make([]*supervised, 0, len(s.jobs))
	for _, sj := range s.jobs {
		if sj.status == scheddomain.JobRunning {
...
func (s *Supervisor) DiscardKind(kind scheddomain.JobKind) {
	s.mu.Lock()
	...
	for _, sj := range toWind {
		...
		if err := s.wind(sj, scheddomain.WindStop); err != nil {
			logger.Warn("wind signal failed", "id", sj.meta.ID, "signal", scheddomain.WindStop.String(), "error", err)
		}
	}
...
func (s *Supervisor) wind(sj *supervised, sig scheddomain.WindSignal) error {
...
func (s *Supervisor) Snapshot() []scheddomain.TrackedJob {
	...
	out := make([]scheddomain.TrackedJob, 0, len(s.jobs))
	for _, sj := range s.jobs {
		tj := scheddomain.TrackedJob{
			Meta:        sj.meta,
			Status:      sj.status,
...
func (s *Supervisor) CountRunning(kind scheddomain.JobKind) int {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Supervisor) IsRunning(id string) bool {
	...
	sj, ok := s.jobs[id]
	return ok && sj.status == scheddomain.JobRunning
}
...
func (s *Supervisor) Cleanup(olderThan time.Duration) int {
	...
	var longRunning []scheddomain.JobMeta
	for id, sj := range s.jobs {
		if !sj.status.IsTerminal() {
...
func (s *Supervisor) Stop() {
	s.stopOnce.Do(func() {
		...
		s.stopped = true
		s.mu.Unlock()
		s.WindAll(scheddomain.WindStop)
	})
	s.wg.Wait()
}

// kindLabel renders a job kind for completion notifications.
func kindLabel(kind scheddomain.JobKind) string {
	switch kind {
