...
type fakeJob struct {
	...
	started chan struct{}
	finish  chan struct{}
	signals []scheddomain.JobSignal

	mu         sync.Mutex
	windCalls  []scheddomain.WindSignal
	closeCalls int
}

func newFakeJob(id string, kind scheddomain.JobKind) *fakeJob {
	return &fakeJob{
		meta:    scheddomain.JobMeta{ID: id, Kind: kind, Label: id, StartedAt: time.Now()},
...
func (f *fakeJob) Meta() scheddomain.JobMeta { return f.meta }

func (f *fakeJob) Run(ctx context.Context, emit func(scheddomain.JobSignal)) agentdomain.ToolExecutionResult {
	close(f.started)
...
func (f *fakeJob) Wind(_ context.Context, sig scheddomain.WindSignal) error {
	f.mu.Lock()
...
func (f *fakeJob) winds() []scheddomain.WindSignal {
	f.mu.Lock()
	defer f.mu.Unlock()
	return append([]scheddomain.WindSignal(nil), f.windCalls...)
}
...
func TestSupervisor_FinishEnqueue(t *testing.T) {
	tests := []struct {
		...
		{name: "non-silent job enqueues once and stays in snapshot", id: "job-1", kind: scheddomain.JobKindShell, wantEnqueue: 1},
		{name: "silent job does not enqueue", id: "silent", kind: scheddomain.JobKindSubagent, silent: true, wantEnqueue: 0},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			snap := sup.Snapshot()
			if len(snap) != 1 || snap[0].Status != scheddomain.JobCompleted {
...
func TestSupervisor_ConcurrentFinishEnqueuesEachOnce(t *testing.T) {
	...
	js := make([]*fakeJob, n)
	for i := range js {
		js[i] = newFakeJob(fmt.Sprintf("job-%d", i), scheddomain.JobKindShell)
		sup.Submit(js[i])
		<-js[i].started
	}
...
func TestSupervisor_WindStopCancelsRun(t *testing.T) {
	...
	job := newFakeJob("j", scheddomain.JobKindSubagent)
	sup.Submit(job)
	<-job.started

	if err := sup.Wind("j", scheddomain.WindStop); err != nil {
		t.Fatalf("Wind: %v", err)
	}
	sup.Stop()

	winds := job.winds()
	if len(winds) == 0 || winds[0] != scheddomain.WindStop {
		t.Fatalf("wind calls = %v, want [stop]", winds)
	}
	snap := sup.Snapshot()
	if len(snap) != 1 || snap[0].Status != scheddomain.JobFailed {
...
func TestSupervisor_WindUnknownJob(t *testing.T) {
	sup := NewSupervisor(&convmocks.FakeMessageQueue{}, &convmocks.FakeConversationRepository{}, nil)
	if err := sup.Wind("missing", scheddomain.WindWrapUp); err == nil {
...
func TestSupervisor_CountRunning(t *testing.T) {
	...
	sub := newFakeJob("subagent", scheddomain.JobKindSubagent)
	sup.Submit(sub)
	<-sub.started

	shell := newFakeJob("shell", scheddomain.JobKindShell)
	sup.Submit(shell)
	<-shell.started

	if got := sup.CountRunning(scheddomain.JobKindSubagent); got != 1 {
...
func TestSupervisor_IsRunning(t *testing.T) {
	...
	job := newFakeJob("j1", scheddomain.JobKindA2A)
	sup.Submit(job)
	<-job.started
...
func TestSupervisor_HasPending(t *testing.T) {
	...
	pane := newFakeJob("pane", scheddomain.JobKindSubagent)
	sup.Submit(pane)
	<-pane.started
	if sup.HasPending() {
	...
	holder := newFakeJob("task", scheddomain.JobKindA2A)
...
func TestSupervisor_DiscardKind(t *testing.T) {
	...
	shell := newFakeJob("shell-1", scheddomain.JobKindShell)
	sup.Submit(shell)
	<-shell.started

	sup.DiscardKind(scheddomain.JobKindA2A)
	...
	if winds := a2a.winds(); len(winds) != 1 || winds[0] != scheddomain.WindStop {
...
func TestSupervisor_DiscardKind_ReapsTerminal(t *testing.T) {
	sup := NewSupervisor(&convmocks.FakeMessageQueue{}, &convmocks.FakeConversationRepository{}, nil)
	job := newFakeJob("t1", scheddomain.JobKindA2A)
	sup.Submit(job)
	<-job.started
	close(job.finish)
	sup.Stop()
...
func TestSupervisor_SubmitAfterStopIgnored(t *testing.T) {
	sup := NewSupervisor(&convmocks.FakeMessageQueue{}, &convmocks.FakeConversationRepository{}, nil)
	sup.Stop()
	sup.Submit(newFakeJob("late", scheddomain.JobKindShell))
	if len(sup.Snapshot()) != 0 {
...
func snapByID(sup *Supervisor, id string) (scheddomain.TrackedJob, bool) {
...
func statusOf(sup *Supervisor, id string) scheddomain.JobStatus {
...
func TestSupervisor_RetentionCountEvictsOldestOnFinish(t *testing.T) {
	sup := NewSupervisor(&convmocks.FakeMessageQueue{}, &convmocks.FakeConversationRepository{}, nil)
	sup.SetRetentionCount(scheddomain.JobKindShell, 2)

	js := make([]*fakeJob, 3)
	for i := range js {
		js[i] = newFakeJob(fmt.Sprintf("shell-%d", i), scheddomain.JobKindShell)
		sup.Submit(js[i])
		<-js[i].started
	}

	close(js[0].finish)
	waitFor(t, func() bool { return statusOf(sup, "shell-0") == scheddomain.JobCompleted })
	time.Sleep(2 * time.Millisecond)
	close(js[1].finish)
	waitFor(t, func() bool { return statusOf(sup, "shell-1") == scheddomain.JobCompleted })
	time.Sleep(2 * time.Millisecond)
	close(js[2].finish)
...
func TestSupervisor_RetentionCountPerKindIndependent(t *testing.T) {
	sup := NewSupervisor(&convmocks.FakeMessageQueue{}, &convmocks.FakeConversationRepository{}, nil)
	sup.SetRetentionCount(scheddomain.JobKindShell, 1)
	sup.SetRetentionCount(scheddomain.JobKindSubagent, 1)

	shell0 := newFakeJob("shell-0", scheddomain.JobKindShell)
	sub0 := newFakeJob("sub-0", scheddomain.JobKindSubagent)
	shell1 := newFakeJob("shell-1", scheddomain.JobKindShell)
	sub1 := newFakeJob("sub-1", scheddomain.JobKindSubagent)
	...
	for i, j := range order {
		close(j.finish)
		waitFor(t, func() bool {
			return statusOf(sup, j.meta.ID) == scheddomain.JobCompleted || absent(sup, j.meta.ID)
		})
		if i < len(order)-1 {
			time.Sleep(2 * time.Millisecond)
		}
...
func TestSupervisor_RunningJobNeverEvicted(t *testing.T) {
	...
	live := newFakeJob("interactive", scheddomain.JobKindSubagent)
	sup.Submit(live)
	<-live.started

	h0 := newFakeJob("headless-0", scheddomain.JobKindSubagent)
	sup.Submit(h0)
	<-h0.started
	h1 := newFakeJob("headless-1", scheddomain.JobKindSubagent)
	sup.Submit(h1)
	<-h1.started

	close(h0.finish)
	waitFor(t, func() bool { return statusOf(sup, "headless-0") == scheddomain.JobCompleted })
	time.Sleep(2 * time.Millisecond)
	close(h1.finish)
	waitFor(t, func() bool { return absent(sup, "headless-0") })

	if j, ok := snapByID(sup, "interactive"); !ok || j.Status != scheddomain.JobRunning {
...
// fakeOutputJob is a fakeJob that also implements scheddomain.JobOutputProvider, returning
...
func TestSupervisor_SnapshotOutput(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			job: func() (scheddomain.BackgroundJob, *fakeJob) {
				inner := newFakeJob("output-shell", scheddomain.JobKindShell)
		...
		},
		{
			...
			job: func() (scheddomain.BackgroundJob, *fakeJob) {
				inner := newFakeJob("no-output", scheddomain.JobKindShell)
				return inner, inner
			},
			wantOutput: "",
		},
...
func TestSupervisor_SnapshotPopulatesOutputAfterCleanupReap(t *testing.T) {
	...
	job := &fakeOutputJob{
		fakeJob: newFakeJob("output-reap", scheddomain.JobKindShell),
