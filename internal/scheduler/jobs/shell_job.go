...
type shellJob struct {
	shell   *scheddomain.BackgroundShell
...
func NewShellJob(shell *scheddomain.BackgroundShell, tracker scheddomain.ShellTracker) scheddomain.BackgroundJob {
	return &shellJob{shell: shell, tracker: tracker}
}

// Meta describes the shell for the task view.
func (j *shellJob) Meta() scheddomain.JobMeta {
	return scheddomain.JobMeta{
		ID:           j.shell.ShellID,
		Kind:         scheddomain.JobKindShell,
...
func (j *shellJob) Run(ctx context.Context, _ func(scheddomain.JobSignal)) agentdomain.ToolExecutionResult {
	...
	case ctx.Err() != nil:
		...
		j.shell.State = scheddomain.ShellStateCancelled
...
func (j *shellJob) Wind(_ context.Context, sig scheddomain.WindSignal) error {
