...
func TestSupervisor_PushesNotifierEvents(t *testing.T) {
	...
	job := newFakeJob("job-1", scheddomain.JobKindShell)
	sup.Submit(job)
	<-job.started
	close(job.finish)
...
func TestSupervisor_SignalEnqueuePushesDrain(t *testing.T) {
	...
	job := newFakeJob("job-sig", scheddomain.JobKindSubagent)
	job.meta.Silent = true
	job.signals = []scheddomain.JobSignal{{Note: "progress", Enqueue: true}}
	sup.Submit(job)
	<-job.started
	close(job.finish)
	sup.Stop()
