...
func startShell(t *testing.T, id, name string, args ...string) *scheddomain.BackgroundShell {
	...
	return &scheddomain.BackgroundShell{
		...
		Cmd:          cmd,
		StartedAt:    time.Now(),
		State:        scheddomain.ShellStateRunning,
...
func waitTerminal(t *testing.T, sup *Supervisor, id string) scheddomain.JobStatus {
	t.Helper()
...
func TestShellJob_WaitsForReadersDoneBeforeReaping(t *testing.T) {
	...
	close(readersDone)
	if got := waitTerminal(t, sup, "s-drain"); got != scheddomain.JobCompleted {
		t.Fatalf("status = %s, want completed", got)
	}
	if shell.State != scheddomain.ShellStateCompleted {
...
func TestShellJob_WindStopUnblocksReadersWait(t *testing.T) {
	...
	if err := sup.Wind("s-stuck", scheddomain.WindStop); err != nil {
		t.Fatalf("Wind: %v", err)
	}
	waitTerminal(t, sup, "s-stuck")
	if shell.State != scheddomain.ShellStateCancelled {
...
func TestShellJob_CompletesAndNotifies(t *testing.T) {
	...
	if got := waitTerminal(t, sup, "s-ok"); got != scheddomain.JobCompleted {
		t.Fatalf("status = %s, want completed", got)
	}
	if shell.State != scheddomain.ShellStateCompleted || shell.ExitCode == nil || *shell.ExitCode != 0 {
...
func TestShellJob_FailureRecordsExitCode(t *testing.T) {
	...
	if got := waitTerminal(t, sup, "s-fail"); got != scheddomain.JobFailed {
		t.Fatalf("status = %s, want failed", got)
	}
	if shell.State != scheddomain.ShellStateFailed || shell.ExitCode == nil || *shell.ExitCode == 0 {
...
func TestShellJob_WindStopCancels(t *testing.T) {
	...
	time.Sleep(20 * time.Millisecond)
	if err := sup.Wind("s-sleep", scheddomain.WindStop); err != nil {
		t.Fatalf("Wind: %v", err)
	}
