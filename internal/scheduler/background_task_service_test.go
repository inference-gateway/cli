...
// (or ctx) and reports A2A polling detail via scheddomain.A2AStateProvider, so it is
...
func TestA2ADivergenceGone(t *testing.T) {
	...
	if c, l := reg.CountRunningJobs(scheddomain.JobKindA2A), len(svc.GetBackgroundTasks()); c != 1 || l != 1 {
		t.Fatalf("while running: CountRunningJobs=%d, GetBackgroundTasks=%d, want 1 and 1", c, l)
	}

	close(job.finish)
	deadline := time.Now().Add(2 * time.Second)
	for (reg.CountRunningJobs(scheddomain.JobKindA2A) != 0 || len(svc.GetBackgroundTasks()) != 0) && time.Now().Before(deadline) {
		time.Sleep(5 * time.Millisecond)
	}
	if c, l := reg.CountRunningJobs(scheddomain.JobKindA2A), len(svc.GetBackgroundTasks()); c != 0 || l != 0 {
