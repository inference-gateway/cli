...
func (c *CustomShortcut) callLLM(ctx context.Context, prompt string) (string, error) {
	...
	snippet, _, err := callLLM(ctx, c.client, model, prompt, customSnippetMaxTokens)
	return snippet, err
}
...
// timeout via ctx.
func callLLM(ctx context.Context, client sdk.Client, model, prompt string, maxTokens int) (string, *sdk.CompletionUsage, error) {
	if client == nil {
...
func loadShortcutsFromFile(filename string, client sdk.Client, modelService convdomain.ModelService, imageService agentdomain.ImageService, toolService agentdomain.ToolService) ([]Shortcut, error) {
