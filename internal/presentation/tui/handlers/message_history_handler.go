...
func (h *MessageHistoryHandler) HandleNavigateBackInTime(event agentdomain.NavigateBackInTimeEvent) tea.Cmd {
	return func() tea.Msg {
		...
		return tui.MessageHistoryReadyEvent{
			Messages: messages,
		}
	}
}
...
func (h *MessageHistoryHandler) HandleEdit(event tui.MessageHistoryEditEvent) tea.Cmd {
	return func() tea.Msg {
		entries := h.conversationRepo.GetMessages()
		...
		return tui.MessageHistoryEditReadyEvent{
...
func (h *MessageHistoryHandler) extractMessages(entries []convdomain.ConversationEntry) []tui.MessageSnapshot {
	messages := make([]tui.MessageSnapshot, 0)
