...
func (sm *StateManager) RecoverFromInconsistentState() error {
	...
	if currentView != tui.ViewStateChat && currentView != tui.ViewStateModelSelection {
		if err := sm.state.TransitionToView(tui.ViewStateChat); err != nil {
			if currentView != tui.ViewStateModelSelection {
				_ = sm.state.TransitionToView(tui.ViewStateModelSelection)
			}
		}
	}
