...
func readinessStateManager(r *tui.AgentReadinessState) *tui.ApplicationState {
...
func TestInputStatusBar_MasterToggle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stateManager := tui.NewApplicationState()
...
func TestInputStatusBar_BuildA2AAgentsIndicator(t *testing.T) {
	tests := []struct {
		name         string
		readiness    *tui.AgentReadinessState
		expectedText string
		expectEmpty  bool
	}{
		{
...
func TestInputStatusBar_SelectionCyclesActionableIndicators(t *testing.T) {
	...
	statusBar.SelectNext()
	if got := statusBar.SelectedAction(); got != tui.StatusIndicatorActionA2AAgents {
