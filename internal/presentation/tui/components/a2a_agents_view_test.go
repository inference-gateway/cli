...
func TestA2AAgentsView_ItemsReflectReadiness(t *testing.T) {
	view, _ := newA2AAgentsViewForTest(&tui.AgentReadinessState{
		TotalAgents: 2,
		ReadyAgents: 1,
...
func TestA2AAgentsView_ResetRefreshesReadiness(t *testing.T) {
	view, stateManager := newA2AAgentsViewForTest(&tui.AgentReadinessState{
		TotalAgents: 1,
		ReadyAgents: 0,
		Agents:      map[string]*tui.AgentStatus{"writer": {Name: "writer", State: agentdomain.AgentStateStarting}},
	})
