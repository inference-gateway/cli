...
func newTestContext(currentView tui.ViewState, inputText string) *keybindingmocks.FakeKeyHandlerContext {
	...
	fakeConversation := &tuimocks.FakeConversationRenderer{}
	fake.GetConversationViewReturns(fakeConversation)

	fakeStatus := &tuimocks.FakeStatusComponent{}
...
func TestActiveActionMembership(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			view:        tui.ViewStateModelSelection,
			inputText:   "",
			actionID:    "tools_toggle_tool_expansion",
			wantPresent: false,
		},
...
func TestKeyResolution(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			registry := keybinding.NewRegistry(nil)
			mockContext := newTestContext(tui.ViewStateChat, tt.inputText)
...
func TestActionHandlers(t *testing.T) {
	registry := keybinding.NewRegistry(nil)
	mockContext := newTestContext(tui.ViewStateChat, "")
...
func TestHelpShortcutGeneration(t *testing.T) {
	registry := keybinding.NewRegistry(nil)
	mockContext := newTestContext(tui.ViewStateChat, "test message")
...
func TestDeleteWordBackwardBindings(t *testing.T) {
	...
	assertResolves := func(t *testing.T, registry *keybinding.Registry) {
		t.Helper()
		ctx := newTestContext(tui.ViewStateChat, "hello world")
