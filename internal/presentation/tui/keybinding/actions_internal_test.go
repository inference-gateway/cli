...
type flashTestCtx struct {
	KeyHandlerContext
	status tui.StatusComponent
	input  tui.InputComponent
}

func (c *flashTestCtx) GetStatusView() tui.StatusComponent { return c.status }
func (c *flashTestCtx) GetInputView() tui.InputComponent   { return c.input }
...
func newFlashCtx(spinnerActive bool, input string) *flashTestCtx {
	...
	in := &tuimocks.FakeInputComponent{}
...
func TestFlashStatusPreservesActiveSpinner(t *testing.T) {
	...
	if _, ok := batch[0]().(tui.SaveStatusStateEvent); !ok {
		t.Errorf("expected the first command to save the status state, got %T", batch[0]())
	}

	ev, ok := batch[1]().(tui.SetStatusEvent)
	if !ok {
...
func TestFlashStatusClearsWhenIdle(t *testing.T) {
	...
	ev, ok := batch[0]().(tui.SetStatusEvent)
	if !ok {
...
func TestHandlePasteEventFlashesAndInserts(t *testing.T) {
	ctx := newFlashCtx(false, "")
	input := ctx.input.(*tuimocks.FakeInputComponent)
...
func (c *historyNavTestCtx) GetInputView() tui.InputComponent           { return c.input }
func (c *historyNavTestCtx) GetAutocomplete() tui.AutocompleteComponent { return c.autocomplete }
...
func TestHandleHistoryDownFocusesStatusBarWhenIdle(t *testing.T) {
	...
	if _, ok := cmd().(tui.FocusStatusBarEvent); !ok {
...
func TestHandleHistoryDownPrefersAutocomplete(t *testing.T) {
	input := &tuimocks.FakeInputComponent{}
	autocomplete := &tuimocks.FakeAutocompleteComponent{}
...
func assertAutocompleteEventText(t *testing.T, cmd tea.Cmd, want string) {
	...
	msg := cmd()
	if ev, ok := msg.(tui.AutocompleteUpdateEvent); ok {
		if ev.Text != want {
	...
	for _, sub := range batch {
		if ev, ok := sub().(tui.AutocompleteUpdateEvent); ok {
			if ev.Text != want {
				t.Fatalf("AutocompleteUpdateEvent.Text = %q, want %q", ev.Text, want)
			}
			return
