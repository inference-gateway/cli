...
conversation store and a deduplicated digest of `~/.infer/logs`, and asks the
model to interpret them - which workflows repeat often enough to deserve a skill,
which tool calls keep failing the same way, and which failures never reached a
session at all. Note that `telemetry.retention_days` (default 7) archives older
files, so an all-time `/insights` still only sees tool totals from the retained
window.
