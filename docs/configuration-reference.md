...
- **client.stall_threshold_sec**: Seconds without progress - no response while connecting, no chunk while streaming - before
  the request counts as stalled (default: `5`, `0` disables). The chat UI shows a reconnecting indicator and the agent drops
  the connection and retries, up to `client.retry.max_attempts` times with exponential backoff
