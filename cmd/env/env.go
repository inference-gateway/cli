...
replace it.`,
		RunE: func(cmd *cobra.Command, args []string) error { return createEnvExample(cmd) },
	}
	command.Flags().Bool("overwrite", false, "Overwrite .env.example if it already exists")
	return command
}
...
func createEnvExample(cmd *cobra.Command) error {
	...
	envExamplePath := ExampleFileName
	...
	if err := os.WriteFile(envExamplePath, []byte(content), 0644); err != nil {
		return fmt.Errorf("failed to create %s file: %w", ExampleFileName, err)
	}
