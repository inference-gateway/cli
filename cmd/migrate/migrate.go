...
schema automatically on connect.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if status {
				return ShowMigrationStatus(state.Config())
			}
			return RunMigrations(state.Config())
		},
	}
	command.Flags().Bool("status", false, "Show migration status without applying migrations")
	return command
}
...
func ShowMigrationStatus(cfg *config.Config) error {
	serviceContainer := container.NewServiceContainer(cfg)
